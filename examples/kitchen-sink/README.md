...
A two-package workspace that exercises **every** covenix feature end to end. It is
...
[`README.md`](../../README.md) is fulfilled. `covenix` is not implemented yet, so
...
kitchen-sink/
  ...
  express-app/             # node module #2 — a typical Express app
    src/
      api.ts               #   the configured Covenix instance + controllers grouped under /v1 (shared)
      main.ts              #   server startup: mount + covenixErrorHandler + listen
...
module reference Zod schemas defined in another — demonstrating that a covenix

 ...
 *
 * covenix reads `req.cookies`, so a cookie parser (e.g. `cookie-parser`) must be
 ...
 * should return a `FileResponse` for this status (covenix streams it); this
 ...
 * generator); covenix sets the SSE headers, frames each yielded value as an event,
 ...
 * the `Covenix` instance's `security` map. Before the handler runs, covenix invokes
 ...
 *
 * @param scheme - The security scheme name (a key in `new Covenix({ security })`).
 ...
 * covenix builds, `@Use` runs **after** `@Security` and **before** multipart parsing

...
/**
 * OpenAPI specification version to emit. covenix produces **`'3.1'`** by default
 ...
 * support (e.g. `openapi-generator`'s `typescript-fetch`); covenix down-converts
...
export interface OpenApiOptions {
   ...
   * `Covenix` instance derives these from its `security` config; pass them
 ...
 * (OpenAPI uses the same curly-brace syntax covenix routes are written in).
 */
...
class DocumentBuilder {
  ...
  private registerStandaloneSchema(schema: ZodType): void {
    if (typeof schema.meta()?.id !== 'string') {
      throw new Error('covenix: schemas passed to registerSchemas must be named via .meta({ id })');
    }
...
export function generateOpenApiDocument(
  ...
  // The builder always emits 3.1 (covenix's native form); down-convert if asked.

...
/** Options common to every zodec response envelope ({@link ResponseBase}). */
...
/**
 * Base class for zodec's value-style responses — return one from a handler to
 ...
 * bodies).
 *
 * zodec detects these by `instanceof`, so returning one is always opt-in — a bare

...
export function assertValidBodyParams(
  ...
  if (!body) {
    ...
    if (offender) {
      throw new Error(
        `zodec: ${injectorLabel(offender)} on "${handlerName}" reads a request body field, but the handler declares no @Body schema`,
      );
    }
    return;
  }
  ...
  for (const param of bodyParams) {
    ...
    if (param.file === 'single') {
      if (fileFields.get(param.name) !== false) {
        throw new Error(
          `zodec: @File('${param.name}') on "${handlerName}" must name a single z.file() field in the @Body schema`,
        );
      }
      continue;
    }
    if (param.file === 'multiple') {
      if (fileFields.get(param.name) !== true) {
        throw new Error(
          `zodec: @Files('${param.name}') on "${handlerName}" must name a z.array(z.file()) field in the @Body schema`,
        );
      }
      continue;
    }
    ...
    if (fields && !fields.has(param.name)) {
      throw new Error(
        `zodec: @BodyParam('${param.name}') on "${handlerName}" names a field absent from the @Body schema`,
      );
    }
  }
}

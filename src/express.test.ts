...
class HelloController {
  ...
  // covenix error handler formats as application/problem+json.
  @Get('bad')
  @Returns(200, Greeting)
...
function api(): Covenix {
  const covenix = new Covenix({ info: { title: 'Test', version: '1.0.0' } });
  covenix.register(new HelloController());
  return covenix;
}
...
describe('toExpress', () => {
  ...
  it('parses JSON bodies by default and serves docs + error handler', async () => {
    ...
    // The default covenix error handler maps a ValidationError to a JSON problem.
  ...
  it('opts out of json, docs, and the error handler when set to false', async () => {
    ...
    // No covenix error handler → Express default (HTML), not a JSON problem.
    const bad = await request(app).get('/hello/bad');
    expect(bad.status).toBe(500);

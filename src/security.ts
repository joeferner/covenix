 ...
 */
export type SecurityHandler = (req: Request, scopes: string[]) => unknown | Promise<unknown>; // eslint-disable-line @typescript-eslint/no-redundant-type-constituents -- keep `Promise<unknown>` visible to document that async handlers are supported (zodec awaits the result)
...
/** The `security` map passed to `new Zodec({ security })`, keyed by scheme name. */
 ...
 * @example
 * ```ts
 * new Zodec({ info, security: { bearerAuth: bearer((req) => verifyJwt(req)) } });
 * ```
 */

...
you register on the `Covenix` instance; before the handler runs, covenix invokes that
...
```typescript
import { Covenix, Route, Get, Security, Principal, Returns, bearer, SecurityError } from 'covenix';
...
- **Returns `null`/`undefined`** — covenix rejects with **`401`**.
- **Throws** — the thrown error propagates (e.g. `new SecurityError(403)` for
  authenticated-but-not-authorized). It can be `async`; covenix awaits it.

`scopes` is the array passed to `@Security(scheme, scopes)` for this route. **The
handler owns the scope check** — covenix doesn't prescribe a principal shape, so
...
```typescript
import { bearer, basic, apiKey, oauth2 } from 'covenix';
...
On an unguarded route `@Principal()` resolves to `undefined`. (covenix doesn't carry
...
covenix runs each in order and injects the first success. If none succeed, the
...
covenix throws [`SecurityError`](/api/classes/SecurityError) (`401`, or `403` when
a handler throws it) through the normal error pipeline. The optional
[`covenixErrorHandler()`](/guide/validation) renders it as an RFC 9457
...
Sharing one `securitySchemes` constant between `new Covenix({ security })` and

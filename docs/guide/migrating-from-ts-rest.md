...
[ts-rest](https://ts-rest.com) and zodec both give you typed routes and runtime
...
- **ts-rest is contract-first and client-first.** You declare a **contract** —
  ...
  opt-in output.
- **zodec is decorator-first and spec-first.** You describe each endpoint with
...
- **The OpenAPI document is the product** — a public API, partner integrations,
  or polyglot/non-TS consumers generating clients. zodec emits accurate **3.1**
...
- **You want validation that can't silently drift.** zodec validates **and
...
zodec folds the contract and the implementation back together on a class — each
...
3. **`return { status, body }` → `return body`.** zodec infers the status from
...
| `headers: { 'x-id': z.string() }`                      | `@Headers(z.object({ 'x-id': z.string() }))` + `@HeaderParam('x-id')` | Both validate request headers; zodec also documents them.      |
...
- **Failure statuses.** ts-rest returns `400` for any request validation failure.
  zodec uses `400` for params/query and **`422`** for body — adjust client
...
- **Multiple schema libraries.** ts-rest accepts any
  ...
  Effect). zodec is **Zod-only** — Valibot/Arktype contracts must be rewritten as
  Zod schemas.
...
zodec inverts the default: every response is validated against its `@Returns`
...
// zodec — always on; the return value is parsed by the @Returns schema
...
the streaming/headers to you on the raw `res`. zodec has dedicated decorators and
response objects that also document themselves in the spec:

- **Upload:** put a `z.file()` field in a `@Body` schema — zodec auto-detects
...
In zodec the spec is the native artifact and needs none of that wiring:
...
security objects to `operationMapper`. zodec makes it first-class: register a
...
directly to zodec's [grouping](/guide/versioning):

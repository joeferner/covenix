...
generator). zodec sets the `text/event-stream` headers, frames each yielded value
as an SSE event, validates it against the schema, and cleans up on disconnect.

```typescript
import { z } from 'zod';
import { Route, Get, Param, Sse, SseEvent } from 'zodec';
...
A `@Sse` route doesn't JSON-respond. zodec:
...
Yield a **plain value** and zodec frames it as a `data:` line (dispatched to the
...
generator's natural teardown). When the client goes away, zodec calls the
...
zodec terminates the stream and surfaces the error through `next(err)` (logged by

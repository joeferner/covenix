...
covenix can generate a **standalone, fully-typed TypeScript client** for your API.
...
```typescript
import { createClient } from './api.gen'; // the generated file — no covenix import
...
`CovenixClientError` (carrying the status and parsed error body) on any non-2xx —
mirroring covenix's server model, where non-2xx are thrown errors:

```typescript
import { CovenixClientError } from './api.gen';

try {
  const user = await api.users.get({ params: { id } }); // user: User
} catch (err) {
  if (err instanceof CovenixClientError && err.status === 404) {
...
`CovenixClientError` likewise carries `.headers` for the thrown path.

## Response shapes

The client handles every response kind covenix can declare:
...
- **Validates response bodies** against the matched status's schema and **request
  ...
  `CovenixClientValidationError` — the underlying `ZodError` is on `.cause`, and
  `.phase` is `'request'` or `'response'`. It's the client-side analogue of
  covenix's always-on **server** response validation, so you catch drift instead of
...
- **It's codegen, not zero-codegen inference.** Unlike ts-rest (whose contract is
  a value the client infers from directly), covenix's contract comes from decorators,
...
- **No React Query / framework hooks** — but the contract is open for a generator
  that emits them ([#28](https://github.com/joeferner/covenix/issues/28)).

...
as [`z.file()`](https://zod.dev) fields. covenix auto-detects the route as
...
field, covenix switches the route to `multipart/form-data` and:
...
backed by the bytes. Configure multer through the `Covenix` constructor — the
